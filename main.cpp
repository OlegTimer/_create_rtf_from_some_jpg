...
jpg to hex by John Smith
https://stackoverflow.com/questions/60948346/how-to-convert-image-file-jpg-to-hex-code-in-c
...
string strimgname0 = imgname0.substr (0,imgname0.size()-4);
string strimgname =  imgname.substr (0,imgname.size()-4);
// MessageBox(0, strimgname0.c_str(), strimgname.c_str(), 0);
	if (strimgname0!=strimgname){
save(filename);
SetWindowText(imgnametext0, imgname0.c_str());
...
std::basic_string<TCHAR> text8;
TCHAR buff8[1024] = {0};
GetWindowText(header_edit, buff8, 1024); 
text8 = buff8;  
header = text8.c_str();
string::size_type index = 0;
void setheadercount(int a);
int a = 1;
while ((index = header.find("\n", index)) != string::npos) {
    header.replace(index, 1, "\\par ");
    ++index; 
	a++;
}
setheadercount(a);
//MessageBox(0, header.c_str(), "h", 0);
...
text = buff;  
address = text.c_str(); 
address += ConvertUTF8ToCP1251(", левая сторона. ");
SetWindowText(adr_edit, address.c_str());
RedrawWindow(hwnd,NULL,NULL,RDW_INVALIDATE );
UpdateWindow(hwnd);
SetFocus(hwnd);	
...
text = buff;  
address = text.c_str(); 
address += ConvertUTF8ToCP1251(", правая сторона. ");
SetWindowText(adr_edit, address.c_str());
RedrawWindow(hwnd,NULL,NULL,RDW_INVALIDATE );
UpdateWindow(hwnd);
SetFocus(hwnd);	
...
std::basic_string<TCHAR> text;
TCHAR buff[1024] = {0};
GetWindowText(bonus_edit, buff, 1024); 
text = buff;  
bonus = text.c_str(); 
bonus += ConvertUTF8ToCP1251(" (не устранено с прошлого/ых объезда/ов  г.).");
SetWindowText(bonus_edit, bonus.c_str());
RedrawWindow(hwnd,NULL,NULL,RDW_INVALIDATE );
UpdateWindow(hwnd);
SetFocus(hwnd);	
...
int getphotow(); int getphotoh();
int photow = getphotow(); int photoh = getphotoh();		
...
double finwd = 2.0*maxsquarepixint/photow; double finhd = 2.0*maxsquarepixint/photoh;

g.ScaleTransform(finhd, finhd);
g.DrawImage(&image, 0, 0);
//MessageBox(0, to_string(finwd).c_str(), to_string(finhd).c_str(), 0);
...
Gdiplus::Image image0(resultw0);
 ...
 //MessageBox(0, to_string(finwd).c_str(), to_string(finhd).c_str(), 0);
g.ScaleTransform(finhd, finhd);
g.DrawImage(&image0, maxsquarepixint*gdiRelativeScreenCoordCoeff -3*maxsquarepixint/4 +10, 0);
}
{
...
x=maxsquarepixint/4+5; y=3*maxsquarepixint/2+maxsquarepixint/10 +5;
...
x+=5*maxsquarepixint/2 - maxsquarepixint/3; y-=maxsquarepixint/10;
HWND hwndButtonle = CreateWindow("BUTTON", " L", 
WS_CHILD | WS_VISIBLE, x, y, maxsquarepixint/6, maxsquarepixint/10, hwnd, (HMENU)110, hInstance, 0);
x+=maxsquarepixint/6;
HWND hwndButtonri = CreateWindow("BUTTON", " R", 
WS_CHILD | WS_VISIBLE, x, y, maxsquarepixint/6, maxsquarepixint/10, hwnd, (HMENU)111, hInstance, 0);

x=5; y+=maxsquarepixint/5;
...
HWND hwndButtonnot = CreateWindow("BUTTON", "Not..", 
WS_CHILD | WS_VISIBLE, x, y+maxsquarepixint/10, maxsquarepixint/5, maxsquarepixint/10, hwnd, (HMENU)112, hInstance, 0);
...
bonus_edit=CreateWindow(TEXT("edit"),TEXT(bonus.c_str()), 
        ...
        x,y,5*maxsquarepixint/2,maxsquarepixint/5,hwnd,(HMENU)5,NULL,NULL );
x=5; y+=maxsquarepixint/5;
...
x=5; y+=maxsquarepixint/10;
CreateWindow(TEXT("STATIC"),TEXT("head"),WS_VISIBLE | WS_CHILD,  x,y,maxsquarepixint/5,maxsquarepixint/10,  hwnd,(HMENU)4,NULL,NULL);
x+=maxsquarepixint/4;
header_edit=CreateWindow(TEXT("edit"),TEXT(header.c_str()), 
        WS_VISIBLE | WS_CHILD | WS_BORDER   |  WS_VSCROLL |   ES_LEFT | ES_MULTILINE | ES_AUTOVSCROLL, 
        x,y,5*maxsquarepixint/2,maxsquarepixint/3,hwnd,(HMENU)5,NULL,NULL );     
